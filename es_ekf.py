...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    """
    Update the posterior mean X and covariance P of the systemt state at time step k

    Args:
    ---
    - sensor_var: variance of sensor --> R: measurement variance
    - p_cov_check: cov of system state
    - y_k: new measurement at time step k
    - p_check: the Mean of predictive distribution of Y
    - v_check: velocity estimation
    - q_check: orientation estimates

    Additive args:
    ---
    - h_jac: measurement matrix

    """
    # 3.1 Compute Kalman Gain
    R = np.identity(3) * sensor_var # measurement covariance
    IS = R+np.dot(h_jac, np.dot(p_cov_check, h_jac.T)) # the Covariance or predictive mean of Y
    K = np.dot(p_cov_check, np.dot(h_jac.T, np.linalg.inv(IS)))

    # 3.2 Compute error state
    error = np.dot(K, (y_k-p_check))
